...
# Function to convert struct to JSON and save to file
function inputdata_to_json_file(input_data::InputData, filename::String)
    json_data = JSON3.write(input_data)
                ...
                if n != "t"
                    ...
                    for scenario in sscens
                        if is_balance_prices
                            ...
                            if isempty(temps)
                                println("Adding column to timeseries_data['scenarios'][$scenario][$k][$series_direction] with series: $series")
                                timeseries_data["scenarios"][scenario][k][series_direction][!, series] = sub_df[!, n]
                            else
                                filtered_df = filter(:t => x -> string(ZonedDateTime(DateTime(x), tz"UTC")) in temps, sub_df)
                                println("Filtered DataFrame for timeseries_data['scenarios'][$scenario][$k][$series_direction] with series: $series, lengths: $(nrow(filtered_df)), $(nrow(sub_df))")
                                timeseries_data["scenarios"][scenario][k][series_direction][!, series] = filtered_df[!, n]
                            end
                        else
                            if !(scenario in keys(timeseries_data["scenarios"]))
                                timeseries_data["scenarios"][scenario] = OrderedDict()
                            ...
                            sub_df = timeseries_data[k]
                            if isempty(temps)
                                println("Adding column to timeseries_data['scenarios'][$scenario][$k] with series: $series")
                                timeseries_data["scenarios"][scenario][k][!, series] = sub_df[!, n]
                            else
                                for t in sub_df.t
                                    println("Checking time: ", t, " - ZonedDateTime: ", string(ZonedDateTime(DateTime(t), tz"UTC")), " - In temps: ", string(ZonedDateTime(DateTime(t), tz"UTC")) in temps)
                                end  
                                filtered_df = filter(:t => x -> string(ZonedDateTime(DateTime(x), tz"UTC")) in temps, sub_df)                              
                                println("Filtered DataFrame for timeseries_data['scenarios'][$scenario][$k] with series: $series, lengths: $(nrow(filtered_df)), $(nrow(sub_df))")
                                timeseries_data["scenarios"][scenario][k][!, series] = filtered_df[!, n]
                            end
                        end
                    end
                end
...
function compile_input_data(system_data::OrderedDict, timeseries_data::OrderedDict, temps::Vector{String}=String[])
                    ...
                    for scenario in sscens
                        if is_balance_prices
                            ...
                            if isempty(temps)
                                ...
                                filtered_df = filter(:t => x -> x in temps, sub_df)
                                timeseries_data["scenarios"][scenario][k][series_direction][!, series] = filtered_df[!, n]
                            end
                        else
                            if !(scenario in keys(timeseries_data["scenarios"]))
    ...
    if length(names(system_data["inflow_blocks"])) > 1
        ...
        for b in collect(keys(inflow_blocks))
            t_col = collect(skipmissing(system_data["inflow_blocks"][!,inflow_blocks[b].name*","*inflow_blocks[b].node]))
            inflow_blocks[b].start_time = t_col[1]
            #inflow_blocks[b].start_time = string(ZonedDateTime(t_col[1], tz"UTC"))
            for s in collect(keys(scens))
                s_col = collect(skipmissing(system_data["inflow_blocks"][!,inflow_blocks[b].name*","*s]))
                if length(t_col) != length(s_col)
                ...
                #series = TimeSeries(s, string.(ZonedDateTime.(t_col, tz"UTC")), s_col)
                series = TimeSeries(s, t_col, s_col)
                push!(inflow_blocks[b].data,series)
            end
        end
    end
    ...
    for i in 1:nrow(system_data["markets"])
        ...
        elseif mm.type == "reserve"
            for s in keys(scens)
                println("Available keys for scenario ", s, ": ", keys(timeseries_data["scenarios"][s]))
                timesteps = timeseries_data["scenarios"][s]["reserve_realisation"].t
                rrs = timeseries_data["scenarios"][s]["reserve_realisation"][!, mm.market]
                ts = Predicer.TimeSeries(s)
                for i in 1:length(timesteps)
    ...
    #use_market_bids = Bool(filter(x -> x.parameter == "use_market_bids", eachrow(system_data["setup"]))[1].value)
    use_market_bids = filter(x -> x.parameter == "use_market_bids", eachrow(system_data["setup"]))[1].value == "true"
    contains_reserves = filter(x -> x.parameter == "use_reserves", eachrow(system_data["setup"]))[1].value == "true"
    #contains_reserves = Bool(filter(x -> x.parameter == "use_reserves", eachrow(system_data["setup"]))[1].value)
    if contains_reserves
        contains_reserves = (true in map(n -> n.is_res, collect(values(nodes))))
    end
    reserve_realisation = filter(x -> x.parameter == "use_reserve_realisation", eachrow(system_data["setup"]))[1].value == "true"
    use_node_dummy_variables = filter(x -> x.parameter == "use_node_dummy_variables", eachrow(system_data["setup"]))[1].value == "true"
    use_ramp_dummy_variables = filter(x -> x.parameter == "use_ramp_dummy_variables", eachrow(system_data["setup"]))[1].value == "true"
    #reserve_realisation = Bool(filter(x -> x.parameter == "use_reserve_realisation", eachrow(system_data["setup"]))[1].value)
    #use_node_dummy_variables = Bool(filter(x -> x.parameter == "use_node_dummy_variables", eachrow(system_data["setup"]))[1].value)
    #use_ramp_dummy_variables = Bool(filter(x -> x.parameter == "use_ramp_dummy_variables", eachrow(system_data["setup"]))[1].value)
    #node_dummy_variable_cost = Float64(filter(x -> x.parameter == "node_dummy_variable_cost", eachrow(system_data["setup"]))[1].value)
    #ramp_dummy_variable_cost = Float64(filter(x -> x.parameter == "ramp_dummy_variable_cost", eachrow(system_data["setup"]))[1].value)
    node_dummy_variable_cost = filter(x -> x.parameter == "node_dummy_variable_cost", eachrow(system_data["setup"]))[1].value === missing ? missing : parse(Float64, filter(x -> x.parameter == "node_dummy_variable_cost", eachrow(system_data["setup"]))[1].value)
    ramp_dummy_variable_cost = filter(x -> x.parameter == "ramp_dummy_variable_cost", eachrow(system_data["setup"]))[1].value === missing ? missing : parse(Float64, filter(x -> x.parameter == "ramp_dummy_variable_cost", eachrow(system_data["setup"]))[1].value)
    #common_timesteps = Int(filter(x -> x.parameter == "common_timesteps", eachrow(system_data["setup"]))[1].value)
    common_timesteps = filter(x -> x.parameter == "common_timesteps", eachrow(system_data["setup"]))[1].value === missing ? missing : parse(Int, filter(x -> x.parameter == "common_timesteps", eachrow(system_data["setup"]))[1].value)
    common_scenario_name_p = filter(x -> x.parameter == "common_scenario_name", eachrow(system_data["setup"]))[1]
    if ismissing(common_scenario_name_p.value)
        common_scenario_name = ""
    else
